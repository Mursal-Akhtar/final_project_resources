"""
Task 4 Variant B: Finer Threshold Grid Only
Uses existing 2-model ensemble (Task 3 SE + Task 3 MHA)
But with finer threshold grid: 0.20-0.80, step 0.02
"""
import os
import json
import numpy as np
...
def run_task4_finer_threshold():
    ...
    val_logits = infer_logits_ensemble([model_se, model_mha], val_loader, device, use_tta=True)

    print("Inferring test set...")
    test_logits = infer_logits_ensemble([model_se, model_mha], test_loader, device, use_tta=True)
    ...
    print(f"\n{'='*70}")
    print(f"TASK 4 VARIANT B: 2-Model Ensemble + Finer Thresholds")
    print(f"{'='*70}")
    print(f"Models: Task3 SE + Task3 MHA (same as original Task4)")
    print(f"Threshold Grid: 0.20-0.80, step 0.02 (finer than original 0.05)")
    ...
    results = {
        "variant": "B: 2-Model Ensemble (Task3-SE + Task3-MHA) + Finer Thresholds",
    ...
    with open("task4_variant_b_results.json", "w") as f:
        json.dump(results, f, indent=2)
    print("Results saved to task4_variant_b_results.json\n")


if __name__ == "__main__":
    run_task4_finer_threshold()
